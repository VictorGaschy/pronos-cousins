...
def obtenir_client_sheets():
    # Récupération des secrets au format gspread
    info_cles = dict(st.secrets["gspread"])
    url_sheet = info_cles.pop("spreadsheet_url") # On extrait l'URL
...
def sauvegarder_pronos_vers_sheet(db):
    try:
        sh = obtenir_client_sheets()
        worksheet = sh.worksheet("pronos")
        worksheet.clear() # On vide pour réécrire proprement
        
        # En-têtes
        lignes = [["Cousin", "Match_ID", "Score1", "Score2"]]
        for cousin, pronos in db.items():
            for match_id, scores in pronos.items():
                lignes.append([cousin, match_id, int(scores["p1"]), int(scores["p2"])])
        
        worksheet.update(range_name="A1", values=lignes)
...
def sauvegarder_reels_vers_sheet(db):
    try:
        ...
        lignes = [["Match_ID", "Score1", "Score2", "Joue"]]
        for match_id, info in db.items():
            lignes.append([match_id, int(info["score1"]), int(info["score2"]), str(info["joue"])])
        
        worksheet.update(range_name="A1", values=lignes)
    except Exception as e:
        st.error(f"Erreur de sauvegarde admin : {e}")
    ...
    if nom_cousin:
        ...
        with st.form(key=f"form_{jour_selectionne}"):
            ...
            if st.form_submit_button("💾 Enregistrer mes pronostics", disabled=bouton_desactive):
                ...
                st.success(f"Impeccable {nom_cousin} ! Enregistré dans Google Sheets.")
                st.rerun()
    else:
        ...
        if choix_admin == "Enregistrer les Scores Réels":
            ...
            with st.form("form_admin"):
                ...
                if st.form_submit_button("📢 Valider et Publier les résultats"):
                    ...
                    st.success("Résultats synchronisés sur Google Sheets !")
                    st.rerun()
            ...
            if not st.session_state.db_pronos:
                ...
                st.write("---")
                st.markdown("### 🔄 Autoriser un cousin à modifier ses scores")
                ...
                if st.button("🔴 Effacer et déverrouiller ce jour"):
                    ...
                    if c_a_modifier in st.session_state.db_pronos:
                        ...
                        sauvegarder_pronos_vers_sheet(st.session_state.db_pronos)
                        st.success(f"Données nettoyées. Le cousin peut rejouer.")
                        st.rerun()
